...
export class AddmodelPage implements OnInit {
  ...
  onSubmit(form: NgForm)
  {
    ...
    this.http.post('https://tapsystock-a6450-default-rtdb.firebaseio.com/car-model.json', {brand:form.value.carbrand, model:form.value.carmodel, startyear:form.value.startyear, endyear:form.value.endyear, icon:iconname}).subscribe(
      resData => {
        if (resData !== undefined || resData !== null)
        {
          console.log(form.value.carmodel);
          form.controls['carmodel'].reset();
